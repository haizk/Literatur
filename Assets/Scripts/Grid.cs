...
public class Grid : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
            {
                ...
                if (Physics2D.OverlapCircle(worldPoint, fNodeRadius - 0.1f, WallMask))
                {
    ...
    {
        Gizmos.color = Color.black;//Set the color of the outside
        Gizmos.DrawWireCube(transform.position, new Vector3(vGridWorldSize.x, vGridWorldSize.y));//Draw a wire cube with the given dimensions from the Unity inspector
